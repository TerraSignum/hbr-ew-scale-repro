...
\path|src/verify_T_parity_from_chirality_pairing.py|).
...
are diagram coefficients with definite physical content --- $D_{1}$
the pure-$\phi$ figure-8 integral, $D_{23}\!=\!D_{2}\!+\!D_{3}$ the
mixed figure-8-tadpole and setting-sun-exchange integrals,
$g_{\mathrm{norm}}$ the two-loop normalisation --- on the
$S^{4}$ Camporesi--Higuchi background. \emph{Provenance note:}
their specific numerical values
($D_{1}\!=\!-0.40893\%$, $D_{23}\!=\!1.842$,
$g_{\mathrm{norm}}\!=\!2.078$) enter this paper as
\emph{stated diagram-coefficient inputs} --- the explicit
two-loop integral evaluations producing them are an external
input to this reproducibility package, not re-derived within it.
What Section~\ref{sec:fourpath} computes is the coupling $|g|$
...
is excluded from the consensus); the four paths do not
independently re-derive $D_{1}$, $D_{23}$, $g_{\mathrm{norm}}$.
The closure is therefore honestly read as: tree-plus-one-loop
$\vEW^{(1\mathrm{L})}$ is rigorously derived here; the two-loop
\emph{correction} is assembled from stated diagram coefficients
and a separately-computed $|g|$.
...
audit \path|src/verify_S_bounce_structural_rational.py|,
output
...
\path|src/audit_eta_S_high_precision.py| and its bundled output
$\Sb=37.995389$ (\path|outputs/audit_eta_S_high_precision.json|)
were committed before the EW-scale matching script
\path|src/recompute_ew_scale.py| consumed $\Sb$ as input.
...
prospective protocol \path|tests/test_saddle_blindness_protocol.py|
...
\footnotesize
\input{tables/tab_S_bounce_audit}
\normalsize
\end{table}
...
\path|src/audit_eta_S_high_precision.py| with the JSON certificate
...
audit (\path|src/audit_eta_S_high_precision.py|, output
...
\path|audit_eta_S_high_precision.py|) at $K\!\in\!\{64,128\}$
...
consistent with $\vEW = 246.22$\,GeV. The second observable
...
of Section~\ref{sec:fourpath} prescribes $|g| = 1.42$.
...
$\vEW = 246.22$\,GeV that this paper closes is also the
...
$\vEW/M_{\mathrm{Pl}}\!=\!246.22/2.435\!\times\!10^{18}\!=\!1.013\!\times\!10^{-16}$,
...
\path|src/verify_post_flip_theories_round3.py| in the
...
\path|src/verify_S_bounce_from_KQ_closed_potential.py| under the
...
(2026-04-25). Convergence audit of three regularization-distinct
