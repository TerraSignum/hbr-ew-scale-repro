...
vs MuLan $246.21965$;
$-5.86\!\times\!10^{-6}$ vs rounded $246.22\bigr)$\hfill\hfill
\normalsize
...
by the integer-uniqueness theorem of~\cite{Paper2}); $T^{*}$ is
...
(\url{https://github.com/[anonymized]/hbr-ew-scale-repro}) reproduces
...
$T^{*}\!=\!1/10$     & \textbf{D} & two-phase coefficient theorem (\cite{Paper6} Thm.~1, \cite{Paper2} readout): $T^{*}\!=\!\gamma^{(V)}\!=\!1/(N_{\rm gen}^{2}\!+\!1)\!=\!1/10$, $N_{\rm gen}\!=\!3$ by integer-uniqueness (\cite{Paper2}) & dimensionless HBR scale (Sec.~\ref{sec:bounce-action}) \\
...
are diagram coefficients computed independently in the four
calculation paths described in Section~\ref{sec:fourpath}.
...
(\url{https://github.com/[anonymized]/hbr-ew-scale-repro}) reproduces both
...
of the companion two-phase paper~\cite{Paper6}, with
$N_{\rm gen}\!=\!3$ fixed by the integer-uniqueness theorem
of~\cite{Paper2}.
...
are available at \url{https://github.com/[anonymized]/hbr-ew-scale-repro}
